node {
    stage('Prep') {
        deleteDir()
        git url: 'https://github.com/mattbrasieratos/listaccounts-mediation'
    ...
    stage('Publish WAR') {
        withCredentials([usernameColonPassword(credentialsId: 'nexus', variable: 'USERPASS')]) {
            sh '''curl -v -u ${USERPASS} --upload-file target/ol001-listaccounts-mediation.war \
                     http://nexus:8081/repository/maven-snapshots/net/atos/ol001-listaccounts-mediation/${BUILD_TIMESTAMP}-SNAPSHOT/ol001-listaccounts-mediation-${BUILD_TIMESTAMP}-SNAPSHOT.war'''
        }
    }
